...
class PublisherService(BaseService):
    ...
    def post_init(self):
        """
        Initialize publisher service class.
        """
        ...
        self.scheduler.add_listener(
            self._process_publishing_result,
    ...
    def _handle_listener_message(self, message):
        ...
        If message is valid initiate publishing of image in new thread.
        """
    ...
    def _handle_service_message(self, message):
        ...
        If message is valid create instance of publisher and
        queue the job.
        ...
        job_config example:
        {
            "publisher_job": {
                ...
                "utctime": "now",
                "publish_regions": {
                    "test-aws": ["us-east-1", "us-east-2", "eu-west-3"],
                    "test-aws-cn": ["cn-north-1", "cn-northwest-1"]
                }
            }
        }
        """
    ...
    def _process_publishing_result(self, event):
        ...
        Handle exceptions and errors that occur during publishing and
        ...
        if event.exception:
            ...
            self.log.error(
                'Pass[{0}]: Exception publishing image: {1}'.format(
                    job.iteration_count,
                    event.exception
                ),
                extra=metata
            ...
            self.log.error(
                'Pass[{0}]: Error occurred publishing image.'.format(
                    job.iteration_count
                ),
                extra=metata
            )
    ...
    def _publish_image(self, job_id):
        """
    ...
    def _validate_listener_msg(self, message):
        ...
        """
        try:
            listener_msg = json.loads(message)['replication_result']
        except Exception:
            self.log.error(
                'Invalid replication result file: {0}'.format(message)
            )
            return None
        ...
        if not job_id:
            self.log.error('id is required in replication result.')
            return None

        job = self.jobs.get(job_id)
        if not job:
            self.log.error(
                'Invalid publisher service job with id: {0}.'.format(job_id)
            )
            return None
        elif status != SUCCESS:
